...
def encode_imval(s):
    ...
    for i in range(0, 32, 2):  # range is [0, 2, 4, ..., 30]
        if (
            rotateleft32(imval, i) < 256
        ):  # -> ans ror i = const with ans 8bit and i 4bit
...
def is_opname(s):
    ...
    return (
        ...
        or is_longmulop(s)
        or is_swiop(s)
        or is_singledatatransop(s)
        or is_halfsigneddatatransop(s)
        or is_swapop(s)
        or is_blockdatatransop(s)
        or is_coprocregtransop(s)
        or is_pseudoinstructionop(s)
...
def is_valid_addresspart(hsflag, addresspart, tflag, address, labeldict):
    ...
    if (
        addresspart[0] != "["
    ):  # must be an expression (label) if not starting with a bracket
    ...
    if (
        len(addresspart) < 1
        or len(addresspart) > 3
        or (hsflag and len(addresspart) > 2)
    ):
    ...
    if is_valid_imval(addresspart[1]):
        ...
        if not preindexed and get_reg_num(addresspart[0]) == get_reg_num(
            addresspart[1]
        ):
...
def encode_singledatatransop(name, flags, condcode, operands, address, labeldict):
    ...
    Encode the instruction and return it as a bytearray object.
    """
    (writeback, preindexed, loadflag, upflag, iflag, rd, rn, offset) = parse_datatrans(
        name, operands, address, labeldict
    )
    if "T" in flags:
        writeback = True
    byteflag = "B" in flags
...
def encode_branchop(name, condcode, operands, address, labeldict):
    ...
    if name == "BX":
        ...
        lflag = name == "BL"
        encoded = encode_32bit(
            [(28, 4, ccval), (25, 3, 0x5), (24, 1, lflag), (0, 24, offset)]
        )
        return bigendian_to_littleendian(encoded)
...
def encode_psrtransop(name, condcode, operands):
    ...
    if name == "MRS":
        ...
        ccval = get_condcode_value(condcode)
        encoded = encode_32bit(
            [(28, 4, ccval), (23, 5, 0x2), (22, 1, spsrflag), (16, 6, 0xF), (12, 4, rd)]
        )
        return bigendian_to_littleendian(encoded)
    else:
...
def encode_miscarithmeticop(name, condcode, operands):
    ...
    ccval = get_condcode_value(condcode)
    encoded = encode_32bit(
        [(28, 4, ccval), (16, 12, 0x16F), (12, 4, rd), (4, 8, 0xF1), (0, 4, rm)]
    )
    return bigendian_to_littleendian(encoded)
...
def encode_op2(op2):
    ...
    if not iflag:
        shiftfield = (
            {"LSL": 0, "ASL": 0, "LSR": 1, "ASR": 2, "ROR": 3}[shifttype.upper()] << 1
        ) | shiftbyreg
        if shiftbyreg:
...
class Sourceline:
    ...
    def _parse_s_suffix(self):
        ...
        for op in soplist:
            if self.opname.startswith(op):
                if (
                    self.opname[:-3] == op
                    and is_condcode(self.opname[-3:-1])
                    and self.opname[-1] == "S"
                ) or (self.opname == op + "S"):
    ...
    def _parse_addrmode_suffixes(self):
        ...
        for op in addrsuffoplist:
            if self.opname.startswith(op):
                if (
                    self.opname[:-4] == op
                    and is_condcode(self.opname[-4:-2])
                    and self.opname[-2:] in addressingmodelist
                ) or (
                    self.opname[:-2] == op and self.opname[-2:] in addressingmodelist
                ):
    ...
    def _check_operation(self):
        ...
        if not is_opname(self.opname + self.flags):
            self.errmsg = "This instruction (name:%s, flags:%s) is unknown" % (
                self.opname,
                self.flags,
            )
            return -1
        if is_conditionable(self.opname + self.flags):
            if len(self.condcode) > 0 and not is_condcode(self.condcode):
                self.errmsg = "Condition code is unknown (condition code: %s)" % (
                    self.condcode
                )
                return -1
        elif len(self.condcode) > 0:
            self.errmsg = (
                "Condition codes are not allowed for this instruction (name:%s, flags:%s, cond:%s)"
                % (self.opname, self.flags, self.condcode)
            )
            return -1
        return 0
    ...
    def _check_label(self):
        ...
        if not is_valid_label(self.label):
            self.errmsg = "This label contains illegal characters (label:%s)" % (
                self.label
            )
            return -1
    ...
    def parse_comments(self):
        ...
        if sci < dqi or dqi == -1:
            ...
            tmpline = self.line.replace(
                '\\"', '""'
            )  # TODO: test #replace c-style escaped " with "", which is the arm asm way of getting a single " inside a string. don't have to care about cases where it's outside a string, because that will be a syntax error later on anyway. sci is still correct.
            while (
                tmpline.count('"', 0, sci) % 2 != 0
            ):  # if the number of quotes before the semicolon is odd, the semicolon is inside a string
    ...
    def replace_pseudoinstructions(self, labeldict):
        ...
        if is_pseudoinstruction(self.opname, self.operands):
            err = check_pseudoinstruction(
                self.opname, self.operands, self.address, labeldict
            )
            if len(err) != 0:
                self.errmsg = err
                return -1
            self.opname, self.operands = get_replacement(
                self.opname, self.operands, self.address, labeldict
            )
        return 0
    ...
    def _check_syntax(self, labeldict):
        ...
        if is_directive(fullname):
            ...
            return check_singledatatransop(
                self.flags, self.operands, self.address, labeldict
            )
    ...
    def _encode_line(self, labeldict):
        ...
        if is_directive(fullname):
            ...
            return encode_dataprocop(
                self.opname, self.flags, self.condcode, self.operands
            )
        elif is_branchop(fullname):
            return encode_branchop(
                self.opname, self.condcode, self.operands, self.address, labeldict
            )
        elif is_psrtransop(fullname):
            ...
            return encode_halfsigneddatatransop(
                self.opname,
                self.flags,
                self.condcode,
                self.operands,
            ...
            return encode_blockdatatransop(
                self.opname, self.flags, self.condcode, self.operands
            )
        elif is_miscarithmeticop(fullname):
    ...
    def assemble(self, labeldict):
        ...
        if len(self.hexcode) != self.length:
            self.errmsg = (
                "Precalculated length (%i bytes) and real length (%i bytes) are not the same"
                % (self.length, len(self.hexcode))
            )
            return -1
        return 0
...
def read_file_and_stage1_parse(infile, filestack=tuple()):
    ...
    if infile in filestack:
        printerror(
            ...
            -1,
            "",
            "file includes itself (possibly indirectly): filestack=%s" % str(filestack),
        )
        numerrs += 1
        return numerrs, []
    text = get_sourcecode()
    ...
    for i, c in enumerate(code):
        ...
        if c.is_incbin():
            ...
            if size < 0:
                printerror(
                    infile, i, c.line, "error in add_file for path '%s'" % (abspath,)
                )
                numerrs += 1
                continue
        ...
        if c.is_include():
            incfile = c.operands
            incnumerrs, inccode = read_file_and_stage1_parse(
                c.operands, filestack=filestack + (infile,)
            )
            numerrs += incnumerrs
            includedcode.append(
                (
...
def assembler(infile, outfile):
    ...
    set_sourcepath(
        ""
    )  # if infile is not an absolute path, it is interpreted relative to current working directory
        ...
        if len(c.label) > 0:
            if c.label in labeldict:
                printerror(
                    ...
                    i,
                    c.line,
                    "Label name already used (at offset 0x%x)" % (labeldict[c.label]),
                )
                numerrs += 1
                continue
    ...
    for i, c in enumerate(code):
        if c.replace_pseudoinstructions(labeldict) == -1:
            if len(c.errmsg) > 0:
                ...
                printerror(
                    infile, i, c.line, "unknown error in replace_pseudoinstructions"
                )
            numerrs += 1
            continue
    if numerrs != 0:
